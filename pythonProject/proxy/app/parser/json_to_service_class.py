...
"""
    ...
    for service_name, service_config in services.items():
        for event_name in service_config.get('events', {}).keys():
            service_code += f"from proxy.app.parser.dataclass.{service_name.lower()}_dataclass import {event_name}Msg\n"
        for method_name in service_config.get('methods', {}).keys():
            service_code += f"from proxy.app.parser.dataclass.{service_name.lower()}_dataclass import {method_name}Msg\n"
...
"""
    ...
    save_code(f'service/{service_name.lower()}_singleton.py', service_code)
    return service_code
